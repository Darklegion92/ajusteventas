...
public class Facturas551 {

    Connection connection;
    String fechaInicialString;
    String fechaFinalString;

    public Facturas551(Connection connection, String fechaInicialString, String fechaFinalString) {
        this.connection = connection;
        this.fechaInicialString = fechaInicialString;
        this.fechaFinalString = fechaFinalString;
    }

    void RecalcularVentas551() {
        try {
            System.err.println("Recalculando Ventas 551");
            ...
            do {
                String sql = "SELECT first 500 fd.fact_id FROM FACTURAS_DETALLE fd, FACTURAS f WHERE fd.fact_id = f.fact_id AND f.fact_fecha  <= ? AND f.fact_fecha >= ? AND f.fact_anulado = 'N' AND fd.fade_tiva = 3";
                ...
                for (String id : lista) {
                    String sql2 = "UPDATE FACTURAS_DETALLE SET fade_tiva = 0, fade_ivaporc=0, fade_ivamonto=0  WHERE fact_id = ? AND fade_tiva = 3";
